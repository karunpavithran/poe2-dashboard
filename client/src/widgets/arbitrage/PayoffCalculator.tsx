...
export const PayoffCalculator = () => {
  ...
  // Resolve the selection against the live (filtered) rows so the seed tracks
  // background refreshes; if the cycle drops out of the current filters it reads
  // as deselected rather than freezing a stale snapshot.
  ...
  // Re-seed only when the selected cycle changes — not on every background
  // refresh — so an in-flight refetch never clobbers the amounts the user typed.
  const [seededKey, setSeededKey] = useState<string | null>(null)
  if (selectedCycleKey !== seededKey) {
    setSeededKey(selectedCycleKey)
    setLegs(selected ? seedLegs(selected) : [])
  }
  ...
  const startCurrency = selected.cycle[0]
  const values = arbitrages.currencyValues
  const profit = [...netByCurrency].reduce(
    (total, [name, net]) => total + net * (values[name] ?? 0),
    0,
  )
  const entrySpent = legs.find(leg => leg.from === startCurrency)?.spent ?? 0
  const investedDivine = entrySpent * (values[startCurrency] ?? 0)
  const profitPct = investedDivine > 0 ? (profit / investedDivine) * 100 : 0

  // Market-depth metrics for the selected cycle (independent of the amounts the
  // user typed): the most Divines/day this cycle can yield at current liquidity,
  // and the gold fee that throughput is already net of (0 until modeled).
  const throughput = cycleThroughputDivine(selected)
  const goldCost = estimateGoldCostDivine(selected)
